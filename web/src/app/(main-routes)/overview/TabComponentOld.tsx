...
const CustomTabHeader = ({ title, img, onClick }: any) => {
  return (
    <div onClick={onClick} className="bg-black-250 py-5 px-4 rounded-lg space-y-3 min-w-[113px] sm:min-w-[150px]">
      <div className="h-7 sm:h-10">
        {img ? <Image className="max-h-7 sm:max-h-10 !w-auto mx-auto" src={img} alt="icon" width={40} height={40} /> : null}
      </div>
...
export default function TabComponentOld({ tabFromQuery, type }) {
  ...
  const defaultMissingClaimColumns = [
    ...
    },
    {
      ...
      cell: ({ cell, row }) => {
        return (
          <div className="min-w-[73px]s flex items-center max-sm:justify-end gap-2">
            {row.original.order_amount ? row.original?.order_amount + ' ' + row.original?.currency : '0,00'}
          </div>
        );
      },
    },
  ...
  return (
    <div>
      <Tabs
        selectedKey={tabFromQuery}
        defaultSelectedKey={tabFromQuery}
        aria-label="Options"
        classNames={{
          base: 'w-full',
          tabList: 'earn-tab-wrapper bg-transparent gap-2.5 sm:gap-5',
          tab: 'text-white !h-auto !p-0 focus:ring-offset-0 focus:ring-0 focus:!outline-0 focus:border-0 focus-visible:!outline-0',
          tabContent: 'group-data-[selected=true]:bg-tertiary-gr p-[2px] rounded-[10px]',
          cursor: 'hidden',
          panel: 'pt-0 sm:pt-10 px-0 pb-0',
        }}
        id="earn-tab"
      >
        <Tab
          ...
          <div>
            <TableWithFilter
              ...
              filterDropdown={
                <ProviderDropdown
                  type={'active_task'}
                  selectedSetting={selectedProvider}
                  setSelectedSetting={setSelectedProvider}
                />
              }
